...
| `code:{num}:proxy` | `proxy_url` | 模型级代理配置 |
...
```
code:{num}:proxy → provider:{name}:proxy → global:proxy → 直连
```

| 层级 | Redis Key | 说明 |
|------|-----------|------|
| 模型级 | `code:{num}:proxy` | 特定模型使用的代理 |
| 提供商级 | `provider:{name}:proxy` | 特定提供商使用的代理 |
| 全局 | `global:proxy` | 默认代理 |
...
-- 提供商代理
providers = {
    anthropic = { baseurl = "...", apikey = "...", proxy = "socks5://127.0.0.1:1080" }
}

-- 模型代理
code = {
    ["01"] = { provider = "anthropic", model = "...", proxy = "http://127.0.0.1:34010" },
    ["02"] = { provider = "anthropic", model = "...", proxy = "" }  -- 强制直连
}
```
